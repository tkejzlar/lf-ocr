...
def find_empty_row(worksheet)
  ...
  for i in 1..100000 do
    cell = players[i][0].value
    return i if cell.value.empty?
  end
end

def add_excel_row(worksheet, first_empty, *columns)
  sheet = @workbook[worksheet]
  formula = sheet[first_empty-1][0].formula
  sheet[first_empty][0].change_contents("", formula)
  columnns.each_with_index do |col, i|
    worksheet[first_empty][i+1].change_contents(col)
  end
end
